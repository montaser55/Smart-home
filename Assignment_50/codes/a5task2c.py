...
def train_and_evaluate_kfold(dataset, foreground_device, best_params, k, epochs, val_split):
...
def main():
    parser = argparse.ArgumentParser(description="Hyperparameter tuning and k-fold evaluation for CNN (open world)")
    ...
    parser.add_argument("--output_plot", type=str, default="precision_recall_plot.png", help="Filename to save the plot")
    ...
    for include_features in [True, False]:
        ...
        for foreground_device in dataset.keys():
            ...
            avg_precision, avg_recall = train_and_evaluate_kfold(
                dataset, foreground_device, best_params, args.k, args.epochs, args.val_split
            )
    ...
    print(f"Precision Recall plot saved as {args.output_plot}")
    plt.show()
