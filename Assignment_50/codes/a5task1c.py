...
def train_and_evaluate_kfold(flows, labels, synthetic_data, best_params, k, epochs, val_split):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(k_splits, 1):
        ...
        X_train = synthetic_data[train_idx]
...
def main():
    parser = argparse.ArgumentParser(description="Hyperparameter tuning and k-fold evaluation for CNN with synthetic data (closed world)")
    parser.add_argument("--main_folder", type=str, required=True, help="Folder containing CSV files")
    parser.add_argument("--m", type=int, required=True, help="Number of flows to take from each device")
    parser.add_argument("--n", type=int, required=True, help="Number of send/receives per flow")
    parser.add_argument("--epochs", type=int, default=30, help="Training epochs")
    parser.add_argument("--val_split", type=float, default=0.2, help="Validation split ratio")
    parser.add_argument("--k", type=int, default=10, help="Number of folds for k-fold cross-validation")
    parser.add_argument("--output_plot", type=str, default="accuracy_comparison_synthetic.png", help="Filename to save the accuracy plot")
    ...
    for include_features in [True, False]:
        ...
        flows, labels, device_to_class = load_data(args.main_folder, args.m, args.n, include_features)
        print("Data loaded")

        print("Generating synthetic data for the entire dataset")
        synthetic_data = []
        for flow in flows:
            synthetic = generate_synthetic_dataset(flow)
            synthetic_data.append(synthetic)
        synthetic_data = np.array(synthetic_data)
        # synthetic_data = generate_synthetic_dataset(flows)
        ...
        y_true_all, y_pred_all = train_and_evaluate_kfold(
            flows, labels, synthetic_data, best_params, args.k, args.epochs, args.val_split
        )

        acc = accuracy_score(y_true_all, y_pred_all)
        accuracies["With Features" if include_features else "Without Features"] = acc
        print(f"Accuracy for include_features={include_features}: {acc:.4f}")

    plt.figure(figsize=(8, 6))
    plt.bar(accuracies.keys(), accuracies.values(), color=['blue', 'orange'])
    plt.title("Comparison of Accuracies with and without Features (Synthetic Data)")
    plt.ylabel("Accuracy")
    plt.ylim(0, 1)
