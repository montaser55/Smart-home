...
# Dictionaries to store data for each protocol
packet_lengths = defaultdict(list)
inter_arrival_times = defaultdict(lambda: defaultdict(list))

# Initialize a pyshark capture object
print(f"Processing file: {file_path}")
capture = pyshark.FileCapture(file_path)

# Previous packet timestamps for each host pair to calculate inter-arrival times
previous_timestamps = {}

# Process each packet in the capture
for packet in capture:
    try:
        # Record the packet length by protocol for all packets
        protocol = packet.highest_layer
        packet_length = int(packet.length)
        packet_lengths[protocol].append(packet_length)

        # Check if packet belongs to Zigbee and extract source/destination addresses
        if hasattr(packet, 'zbee_nwk'):
            ...
            # Filter inter-arrival times based on specified device addresses
...
# Calculate and print statistics for packet lengths
...
# Calculate and print statistics for inter-arrival times only for relevant device communications
